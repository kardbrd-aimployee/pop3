...
use crate::render::model::{IterTriangleModel, RefToTriangle, Triangle, TriangleIteratorVector3};

pub fn intersect_triangle<V>(vs: &Vector3<V>, ve: &Vector3<V>, t1: &Triangle<V>) -> (bool, V)
where
    V: Float,
{
    let t = t1;
...
fn mul<V>(vec: &Vector3<V>, m: &Matrix4<f32>) -> Vector3<V>
where
    V: Float,
{
    let vec_1: Vector3<f32> = Vector3 {
        x: <f32 as NumCast>::from(vec.x).unwrap(),
        y: <f32 as NumCast>::from(vec.y).unwrap(),
        z: <f32 as NumCast>::from(vec.z).unwrap(),
    };
    let vec_2 = (m * vec_1.extend(one::<f32>())).truncate();
    Vector3 {
        x: <V as NumCast>::from(vec_2.x).unwrap(),
        y: <V as NumCast>::from(vec_2.y).unwrap(),
        z: <V as NumCast>::from(vec_2.z).unwrap(),
    }
}

pub fn intersect<'a, M, V>(
    model: &'a M,
    mvp: &Matrix4<f32>,
    vec_s: Vector3<V>,
    vec_e: Vector3<V>,
...
where
    M: IterTriangleModel<'a, Vector3<V>>,
    V: Float + 'a,
...
where
    I: Iterator<Item = (usize, Triangle<V>)>,
    V: Float,
{

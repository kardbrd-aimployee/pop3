...
impl<V, I> VertexModel<V, I> for MeshModel<V, I>
where
    I: FromUsize,
{
    fn push_vertex(&mut self, v: V) -> I {
...
pub trait TriangleModel<V, I>
where
    Self: VertexModel<V, I>,
{
...
where
    ...
    I: 'a + FromUsize,
{
    type Iter = TriangleIterator<
        'a,
        V,
        I,
        Zip<RangeFrom<usize>, StepBy<Range<usize>>>,
        Zip<RangeFrom<usize>, Chunks<'a, V>>,
    >;
    ...
    fn iter(&'a self) -> Self::Iter {
        ...
        TriangleIterator {
            mesh: self,
            is_indices,
            iter_internal_indices: i,
            iter_internal_vertices: v,
        }
    }
}
        ...
        if self.is_indices {
            match self.iter_internal_indices.next() {
                Some((n, i)) => {
                    ...
                    let t: TriangleRef<V> = TriangleRef {
                        a: &self.mesh.vertices[i1],
...
pub trait RefToTriangle<V> {
    ...
    fn to_triangle(t: &TriangleRef<Self>) -> Triangle<V>
    where
        Self: Sized,
    {
        let a = t.a.to();
...
where
    Iter: Iterator<Item = (usize, TriangleRef<'a, V>)>,
    ...
    V: RefToTriangle<V2> + 'a,
{
    ...
    pub fn from_model<M>(model: &'a M) -> Self
    where
        M: IterTriangleModel<'a, V, Iter = Iter>,
    {
        let iter = model.iter();
        Self::new(iter)
    }

...
- `FUN_004018a0` checks the construction element's visible field at `+0x10` before composing its HFX nine-patch. A hidden command draws neither a frame nor a glyph, leaving uninterrupted tiled panel texture; the native Level 1 grid consequently shows only hut, warrior-training, and firewarrior controls.
- `GUI_RenderTiledPanel` at `0x004936B0` composes panel surfaces from `hfx0-0.dat` sprites `1450..1465`, choosing distinct corners, alternating edges, and four alternating interior pieces. These sprites use the active landscape palette rather than the POINT palette.
...
- `Building_InitModelSelector` at `0x004364E0` chooses native model selector `107`, `119`, or `131` for a blue small hut, then adds `3 * tribe + subtype - 1`. The loaded native model table is based 38 records into the raw OBJS0 archive, so those selectors correspond to file entries `145`, `157`, and `169` used by the Rust extractor. Each family remains selected throughout construction, completion, and destruction.
...
- Construction phase at object offset `+0x78` runs from zero through four. The selected hut mesh does not translate vertically; its secondary face flags select phase sets. The transition handler around `0x0049EAC0`/`0x0049EDD7` also emits temporary render-type-10 face objects, which is why a stable draw of every raw phase mask does not match the recorded bare-cage-to-skinned-hut animation.
- `Building_OnConstructionComplete` at `0x0042FD70` sets phase four and switches render type from `10` to the type-table value `5`, while passing the existing model selector at `+0x63` back to `Object_SetShapeFromType`. It therefore draws the complete selected mesh rather than replacing it with a separate model.
- Building destruction runs the same face-mask phases in reverse before the remaining object sinks and is removed.
- Canonical building angle quadrants are stored directly as `0`, `512`, `1024`, and `1536`; their entrance directions are `-Z`, `-X`, `+Z`, and `+X`. The Rust renderer preserves those gameplay angles and applies a separate `-PI/2` model-space correction because the extracted OBJS axes otherwise appear 90 degrees counterclockwise.
...
- Raw OBJS0 archive entries `145`, `146`, and `147` are the first blue hut family's small, medium, and large meshes (native selectors `107`, `108`, and `109`). Construction and renovation reveal the selected family mesh by phase masks rather than using separate scaffold models.
